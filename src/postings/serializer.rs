...
impl<W: Write> PostingsSerializer<W> {
    pub fn new(
        ...
        let avg_fieldnorm: f32 = fieldnorm_reader.as_ref().map(get_avg_fieldnorm).unwrap_or(0f32);
        let num_docs = fieldnorm_reader.as_ref().map(|fieldnorm_reader| fieldnorm_reader.num_docs()).unwrap_or(0u32);
        PostingsSerializer {
    ...
    pub fn new_term(&mut self, term_doc_freq: u32) {
        ...
        if self.num_docs > 0 {
            let bm25_weight = BM25Weight::for_one_term(term_doc_freq as u64, self.num_docs as u64, self.avg_fieldnorm);
        ...
        if self.termfreq_enabled {
            ...
            if let Some(bm25_weight) = self.bm25_weight.as_ref() {
                if let Some(fieldnorm_reader) = self.fieldnorm_reader.as_ref() {
                    ...
                    blockwand_params_opt = docs.iter().cloned().map(|doc| fieldnorm_reader.fieldnorm_id(doc))
                        .zip(term_freqs.iter().cloned())
                        .max_by(|(left_fieldnorm_id, left_term_freq), (right_fieldnorm_id, right_term_freq)| {
                            let left_score = bm25_weight.tf_factor(*left_fieldnorm_id, *left_term_freq);
                            let right_score = bm25_weight.tf_factor(*right_fieldnorm_id, *right_term_freq);
